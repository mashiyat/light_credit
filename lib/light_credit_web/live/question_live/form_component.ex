defmodule LightCreditWeb.QuestionLive.FormComponent do
  ...
  def update(%{question: question} = assigns, socket) do
    changeset = CreditEvaluation.change_question(question)
  ...
  def handle_event("validate", %{"question" => question_params}, socket) do
    changeset =
      ...
      |> CreditEvaluation.change_question(question_params)
  ...
  defp save_question(socket, :edit, question_params) do
    case CreditEvaluation.update_question(socket.assigns.question, question_params) do
      {:ok, _question} ->
        {:noreply,
         socket
  ...
  defp save_question(socket, :new, question_params) do
    case CreditEvaluation.create_question(question_params) do
      {:ok, _question} ->
        {:noreply,
         socket
